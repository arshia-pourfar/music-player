...
const SearchBox = ({ widthSize, titleText }) => {
    ...
    return (
        <div className='flex justify-between items-center'>
            ...
            <i className='fi fi-rs-bars-staggered lg:hidden flex justify-center items-center lg:text-5xl text-3xl'></i>
            <div className='bg-black lg:hidden inline-block w-[100dvw] h-screen absolute left-0 bottom-0 z-[1000]'>
                <Navbar />
            </div>
            <div
                className={isClickIcon ? 'inline w-[100dvw] h-[100dvh] absolute left-0 top-0 z-[990] bg-transparent' : 'hidden'}
                onClick={() => setIsClickIcon(false)}
            ></div>
            <div
                id='search-box'
                className={`flex items-center lg:justify-end justify-between z-[999] ${widthSize ? 'xl:w-[700px] lg:w-[600px] md:w-full' : 'xl:w-[600px] lg:w-[500px] md:w-full'}`}
            >
                ...
                <form className='search-box relative flex lg:justify-normal md:justify-between w-[95%]' action='#'>
                    <div className='flex lg:w-full w-[80%] bg-custom-white rounded-lg shadow-md border'>
                        <label htmlFor="search-music" className='flex items-center justify-center'>
                            <i className='fi fi-rs-search px-3 py-1.5 mt-1'></i>
                        </label>
                        <input
                            type="text"
                            className='lg:w-11/12 w-full bg-custom-white outline-none text-[15px]'
                            id='search-music'
                            placeholder='Search'
                        />
                    </div>
